...
async function refresh(){try{const response=await fetch('/api/stats');if(!response.ok)throw Error(response.status);const d=await response.json(),j=(d.jobs||[])[0];if(!j){runStatus.textContent='idle';statusDot.className='dot bad';return}const done=n(j.today),goal=n(j.daily_target),pct=goal?Math.min(100,done/goal*100):0,rate=n(j.rate_per_second),dupes=n(j.duplicates);title.textContent=`${j.query} 采集进度`;campaignId.textContent=j.id;runStatus.textContent=statusText[j.status]||j.status;statusDot.className=j.status==='active'?'dot':'dot bad';today.textContent=fmt(done);target.textContent=fmt(goal);percent.textContent=`${pct.toFixed(3)}%`;progress.style.width=`${pct}%`;discovered.textContent=fmt(j.discovered);duplicateRate.textContent=`${((dupes/Math.max(1,done+dupes))*100).toFixed(1)}%`;speed.textContent=`${(rate*60).toFixed(1)} 条/分钟`;remaining.textContent=eta(Math.max(0,goal-done),rate);collector.textContent=j.proxy_profile==='private'?'private-proxy':j.proxy_profile;jobStatus.textContent=statusText[j.status]||j.status;health.textContent=j.status==='failed'?'error':'ok';health.className=j.status==='failed'?'bad-text':'ok';failed.textContent=fmt(j.failed);const event=(d.events||[])[0];anomaly.textContent=event?`${event.error_code||event.status} · ${ago(event.created_at)}`:'—';const pools=d.proxy_pools||{};proxyRows.innerHTML=['private','public'].map(name=>{const p=pools[name]||{},usable=!!p.usable;return`<tr><td>${name==='private'?'私有代理池':'公共代理池'}</td><td>${fmt(p.fresh)}</td><td>${fmt(p.http)}</td><td>${fmt(p.socks5)}</td><td>${ago(p.synced_at)}</td><td class="${usable?'ok':'bad-text'}">${usable?'ok':'unavailable'}</td></tr>`}).join('');updated.textContent=`索引 ${fmt(d.indexed_documents)} 条 · 最近刷新 ${new Date().toLocaleTimeString()}`}catch(e){runStatus.textContent='offline';statusDot.className='dot bad';updated.textContent='统计读取失败，正在重试'}}refresh();setInterval(refresh,2000);
</script></body></html>'''
