...
</style></head><body><main><h1>实时网页搜索爬虫</h1><div class="sub">搜索引擎实时发现 URL → 并发访问当前网页 → 提取正文并建立索引</div><form class="crawl" id="crawlForm"><input id="query" placeholder="输入关键词，例如：新加坡 人工智能 最新政策" required><select id="pages"><option value="1">1 页结果</option><option value="3" selected>3 页结果</option><option value="5">5 页结果</option><option value="10">10 页结果</option></select><button>开始实时抓取</button></form><div class="cards"><div class="card"><span>索引网页总数</span><b id="total">0</b></div><div class="card"><span>本次发现</span><b id="discovered">0</b></div><div class="card"><span>实时成功</span><b id="fetched">0</b></div><div class="card"><span>失败</span><b id="failed">0</b></div><div class="card"><span>robots 拒绝</span><b id="blocked">0</b></div></div><div class="status" id="jobStatus">等待任务</div><form class="local" id="searchForm"><input id="searchQuery" placeholder="搜索已经实时抓取的正文"><button>搜索索引</button></form><div id="results" class="empty">抓取完成后会显示当前网页内容</div><div class="warn">不使用 Common Crawl 或付费 API。免费搜索引擎和目标网站仍可能限流；动态 JavaScript、登录页面可能无法提取。</div></main><script>
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let active='';
async function stats(){const d=await fetch('/api/stats').then(r=>r.json());total.textContent=d.indexed_documents.toLocaleString();const j=active?d.jobs.find(x=>x.id===active):d.jobs[0];if(j){discovered.textContent=j.discovered;fetched.textContent=j.fetched;failed.textContent=j.failed;blocked.textContent=j.blocked;jobStatus.textContent=`任务 ${j.status} · 关键词“${j.query}” · ${new Date(j.updated_at).toLocaleString()}`;if(j.status==='completed'&&active){active='';searchQuery.value=j.query;runSearch(j.query)}}}
...
crawlForm.onsubmit=async e=>{e.preventDefault();const q=query.value.trim();const r=await fetch('/api/crawl',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:q,pages:Number(pages.value),workers:12})});const d=await r.json();if(!r.ok){jobStatus.textContent=d.error;return}active=d.job_id;jobStatus.textContent='任务已创建，正在实时发现 URL…';searchQuery.value=q};searchForm.onsubmit=e=>{e.preventDefault();runSearch(searchQuery.value.trim())};stats();setInterval(stats,2000);
</script></body></html>'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            pages = min(max(int(payload.get("pages", 3)), 1), 10)
            workers = min(max(int(payload.get("workers", 12)), 1), 32)
            if not query or len(query) > 300:
                raise ValueError("关键词不能为空且不能超过 300 字符")
            job_id = self.coordinator.start(query, pages, workers)
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/":
            self.send_body(SIMPLE_HTML.encode(), "text/html; charset=utf-8")
            return
        ...
        if parsed.path == "/api/stats":
            payload = self.coordinator.store.stats()
