...
class FreeGoogleTests(unittest.TestCase):
    ...
    def test_chinese_wml_shell_without_empty_phrase_is_confirmed_empty(self):
        query = "site:example.com 人工智能"
        html = (
            '<html><head><title>' + query + ' - Google 搜索</title></head><body>'
            '<a href="/search?q=x&amp;tbm=isch">图片</a>'
            '<a href="/url?q=https%3A%2F%2Fsupport.google.com%2Fwebsearch&amp;sa=U">了解详情</a>'
            '</body></html>'
        )
        session = Mock()
        response = session.get.return_value
