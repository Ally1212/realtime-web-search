...
- Whale 的 `published_at` 是必填项（预检实测 HTTP 400）。优先使用原站明确带时区的 `datePublished`/`article:published_time`；缺失时用正文采集时间兜底，并在消息元数据标记 `collector:fetched_at`。本地原站发布时间字段不写入兜底值。
